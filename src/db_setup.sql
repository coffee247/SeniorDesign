...
insert into Environment (humidityPercent, tempFloat, barroPressFloat)
...
INSERT INTO ballisticians(ballistician)
VALUES ('Luke Jeter'),('James Stallings'),('Micah Parks');
...
create Table querries (Descr varchar(256) primary key,
TheQuery varchar(512));

insert into querries(Descr, TheQuery)
values ('Show all shots (order by date)', 'select * from shot order by ShotDate ASC'),
       ('Show all shots (order by descending date)', 'select * from shot order by ShotDate DESC'),
       ('List 44 mag (order by mass ASC)', 'select * from projo where projectileType like \'%44%ag%\' order by projo_mass asc');
...
CREATE TABLE projo (projectileID int(4) key unique not null, projectileType varchar(20),
projo_mass float,
...
INSERT INTO fabrics (fabricType)
VALUES ('Kevlar Ballistic Fiber');
